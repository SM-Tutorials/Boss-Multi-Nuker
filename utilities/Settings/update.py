...
def search_for_updates():
    ...
    r = requests.get("https://github.com/SM-Tutorials/Boss-Multi-Nuker/releases/latest")
    ...
    if THIS_VERSION not in result_string:
        ...
        print(f'''\n\n                               [{lr}!{w}] Boss-Nuker [{m}{THIS_VERSION}{w}] is OUTDATED''')
        soup = BeautifulSoup(requests.get("https://github.com/SM-Tutorials/Boss-Multi-Nuker/releases").text, 'html.parser')
        ...
        if choice.lower() == 'y' or choice.lower() == 'yes':
            print(f"\n                               [{g}#{w}] Updating Boss-Nuker...")

            if os.path.basename(sys.argv[0]).endswith("exe"):
                with open("Boss-Multi-Nuker.zip", 'wb')as zipfile:
                    zipfile.write(requests.get(update_url).content)
                with ZipFile("Boss-Multi-Nuker.zip", 'r') as filezip:
                    filezip.extractall()
                os.remove("Boss-Multi-Nuker.zip")
                cwd = os.getcwd()+'\\Boss-Multi-Nuker\\'
                shutil.copyfile(cwd+'Changelog.md', 'Changelog.md')
                try:
                    shutil.copyfile(cwd+os.path.basename(sys.argv[0]), 'Boss-Multi-Nuker.exe')
                except Exception:
                    pass
                shutil.copyfile(cwd+'README.md', 'README.md')                   
                shutil.rmtree('Boss-Multi-Nuker')
                input(f"                               [{g}#{w}] Update Successfully Finished!", end="")
                os.startfile("Boss-Multi-Nuker.exe")
                os._exit(0)

            else:
                new_version_source = requests.get("https://github.com/SM-Tutorials/Boss-Multi-Nuker/archive/refs/heads/master.zip")
                with open("Boss-Multi-Nuker.zip", 'wb')as zipfile:
